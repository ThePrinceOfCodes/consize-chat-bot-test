...
export const acceptInvite = async (token: any, password: string): Promise<IUserDoc> => {
  try {
    const resetPasswordTokenDoc = await verifyToken(token, tokenTypes.RESET_PASSWORD)
    let user = await getUserById(resetPasswordTokenDoc.user)
    if (!user) {
      throw new Error()
    }
    user = await updateUserById(user.id, { password, isEmailVerified: true })
    if (!user) {
      throw new Error()
    }
    await Token.deleteMany({ user: user.id, type: tokenTypes.RESET_PASSWORD })
