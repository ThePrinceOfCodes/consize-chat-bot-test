...
describe('paginate plugin', () => {
  describe('populate option', () => {
    test('should populate the specified data fields', async () => {
      ...
      expect(taskPages.data[0]).toMatchObject({ project: { _id: project._id, name: project.name } })
    })
  })
  ...
  describe('sortBy option', () => {
    test('should sort results in ascending order using createdAt by default', async () => {
      ...
      expect(projectPages.data).toHaveLength(3)
      expect(projectPages.data[0]).toMatchObject({ name: projectOne.name })
      expect(projectPages.data[1]).toMatchObject({ name: projectTwo.name })
      expect(projectPages.data[2]).toMatchObject({ name: projectThree.name })
    })
    ...
    test('should sort results in descending order if descending sort param is specified', async () => {
      ...
      expect(projectPages.data).toHaveLength(3)
      expect(projectPages.data[0]).toMatchObject({ name: projectThree.name })
      expect(projectPages.data[1]).toMatchObject({ name: projectTwo.name })
      expect(projectPages.data[2]).toMatchObject({ name: projectOne.name })
    })
  })
  ...
  describe('limit option', () => {
    ...
    test('should limit returned results if limit param is specified', async () => {
      ...
      expect(projectPages.data).toHaveLength(2)
      expect(projectPages.data[0]).toMatchObject({ name: 'Project One' })
      expect(projectPages.data[1]).toMatchObject({ name: 'Project Two' })
    })
  })
  ...
  describe('page option', () => {
    ...
    test('should return the correct page if page and limit params are specified', async () => {
      ...
      expect(projectPages.data).toHaveLength(1)
      expect(projectPages.data[0]).toMatchObject({ name: 'Project Three' })
    })
  })
  ...
  describe('projectBy option', () => {
    ...
    test('should exclude a field when the hide param is specified', async () => {
      ...
      expect(projectPages.data[0]).not.toMatchObject({ milestones: expect.any(Number) })
    })
    ...
    test('should exclude multiple fields when the hide param is specified', async () => {
      ...
      expect(projectPages.data[0]).not.toMatchObject({ milestones: expect.any(Number), name: expect.any(String) })
    })
    ...
    test('should include a field when the include param is specified', async () => {
      ...
      expect(projectPages.data[0]).not.toMatchObject({ name: expect.any(String) })
      expect(projectPages.data[0]).toMatchObject({ milestones: expect.any(Number) })
    })
    ...
    test('should include multiple fields when the include param is specified', async () => {
      ...
      expect(projectPages.data[0]).toHaveProperty('milestones')
      expect(projectPages.data[0]).toHaveProperty('name')
    })
    ...
    test('should always include id when the include param is specified', async () => {
      ...
      expect(projectPages.data[0]).not.toMatchObject({ name: expect.any(String) })
      expect(projectPages.data[0]).toMatchObject({ id: expect.any(String), milestones: expect.any(Number) })
    })
  })
})
